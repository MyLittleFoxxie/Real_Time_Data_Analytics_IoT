...
Importable everywhere (pure stdlib, no pandas / no pyspark) so the producer,
the Spark job, and the offline validator all agree on the *exact* same
windowing parameters and synthesized timestamps. Determinism here is what lets
`src/validate.py` reproduce Spark's alerts byte-for-byte.
"""
...
# It is below threshold, so it never produces an alert; the producer and the
# validator emit it identically, keeping their results in exact agreement.
FLUSH_GAP_SECONDS = 600            # 10 min after the last real reading
SENTINEL_HR = 70.0                 # benign heart rate for the flush tick

# Fixed base so the producer and the validator generate identical timestamps.
...
def window_start_for(ts: _dt.datetime) -> _dt.datetime:
    """Tumbling-window start that Spark's window('2 minutes') assigns to `ts`.

    Spark anchors tumbling windows at the Unix epoch, so the start is the
    timestamp floored to the nearest WINDOW_SECONDS boundary.
    """
    epoch = _dt.datetime(1970, 1, 1, tzinfo=_dt.timezone.utc)
    secs = int((ts - epoch).total_seconds())
    floored = secs - (secs % WINDOW_SECONDS)
    return epoch + _dt.timedelta(seconds=floored)
